...
/// - [`state::Idle`]: the client has been initialized.
/// - [`state::Subscribed`]: the application is ready to handle logs and updates, but the process is not
///   running yet
/// - [`state::Active`]: the client is actively fetching data and may now handle requests.
#[derive(Debug)]
