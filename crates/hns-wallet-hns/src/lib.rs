...
impl<B: HnsBackend, C: HnsClock> HnsWalletRuntime<B, C> {
    ...
    pub fn rebroadcast_dropped_pending_sends(
        ...
        for stored in workflows {
            ...
            let observed_stage = if evidence.status.conflicted || competing_spender {
                Some(HnsSendStage::Conflicted)
            } else if evidence.status.confirmation_count > 0 {
                Some(HnsSendStage::Confirmed)
            } else if evidence.status.in_mempool {
                Some(HnsSendStage::Mempool)
            } else {
                None
            };
            if let Some(observed_stage) = observed_stage {
                let mut store = self.store_lock()?;
                persist_observed_send_stage(&mut store, &account, &stored, observed_stage, now)?;
                continue;
            }
            if send_is_within_propagation_grace(stored.state.stage, stored.updated_at_unix, now) {
                continue;
            }
            dropped.insert(txid);
    ...
    fn persist_prepared_settlement(
        ...
        let mut store = self.store_lock()?;
        let expected_workflow_revision =
            match store.load_workflow::<HnsPreparedSettlement>(workflow_id)? {
                ...
                Some(stored)
                    if stored.kind == kind
                        && stored.state.wallet_id == prepared.wallet_id
                        && stored.state.account_id == prepared.account_id
                        && stored.state.workflow_id == prepared.workflow_id
                        && stored.state.session_id == prepared.session_id
                        && stored.state.action == prepared.action
                        && settlement_terms_are_retry_compatible(
                            &stored.state.terms,
                            &prepared.terms,
                        )
                        && ((matches!(
                            ...
                            || (stored.state.stage == HnsSettlementStage::Prepared
                                && stored.state.expires_at_unix <= now_unix)) =>
                {
                    if account_input_reservations(&store, &account.config)?
                        .into_iter()
    ...
    fn prepare_settlement_spend(
            ...
            .map_err(map_chain_error)?
        {
            if stored.kind != settlement_workflow_kind(action)
                ...
                || !settlement_terms_are_retry_compatible(&stored.state.terms, &terms)
            {
                return Err(ChainError::InvalidRequest(
            ...
            if !active_retry {
                ...
                if stored.state.stage == HnsSettlementStage::Prepared {
                    ...
                    release_reservations(&mut store, &config, workflow_id)
                        .map_err(map_chain_error)?;
                }
            } else {
                if stored.state.maximum_fee != maximum_fee || stored.state.fee > maximum_fee {
                    return Err(ChainError::InvalidRequest(
                        "persisted Handshake settlement spend does not match retry",
                    ));
                }
...
fn persist_observed_send_stage(
    store: &mut WalletStore,
    config: &HnsRuntimeConfig,
    expected: &StoredWorkflow<HnsSendWorkflow>,
    next_stage: HnsSendStage,
    now_unix: u64,
) -> Result<(), HnsWalletError> {
    if !matches!(
        next_stage,
        HnsSendStage::Mempool | HnsSendStage::Confirmed | HnsSendStage::Conflicted
    ) || expected.state.plan.wallet_id != config.wallet_id
        || expected.state.plan.account_id != config.account_id
        || expected.state.plan.workflow_id != expected.id
        || !matches!(
        ...
        || expected.state.transaction.is_none()
        || expected.state.signed_transaction.is_none()
    {
        return Err(HnsWalletError::InvalidWorkflow);
    }
    if expected.state.stage == next_stage {
        return Ok(());
    }
    let current = store
        .load_workflow::<HnsSendWorkflow>(expected.id)?
        .ok_or(HnsWalletError::InvalidWorkflow)?;
    ...
    let deletes = if matches!(
        ...
        reservation_deletes(store, config, expected.id)?
    } else {
        Vec::new()
    };
    store.save_workflow_with_entity_batch::<_, HnsInputReservation>(
...
fn settlement_terms_are_retry_compatible(
    stored: &HnsSettlementTerms,
    candidate: &HnsSettlementTerms,
) -> bool {
    match (stored, candidate) {
        (
            HnsSettlementTerms::Lock { request: stored },
            HnsSettlementTerms::Lock { request: candidate },
        ) => {
            stored.session_id == candidate.session_id
                && stored.module == candidate.module
                && stored.amount == candidate.amount
                && stored.hashlock == candidate.hashlock
                && stored.receiver == candidate.receiver
                && stored.refund_target == candidate.refund_target
                && stored.absolute_timelock == candidate.absolute_timelock
        }
        (
            HnsSettlementTerms::Redeem { lock: stored },
            HnsSettlementTerms::Redeem { lock: candidate },
        )
        | (
            HnsSettlementTerms::Refund { lock: stored },
            HnsSettlementTerms::Refund { lock: candidate },
        ) => {
            stored.module == candidate.module
                && stored.session_id == candidate.session_id
                && stored.funding_id == candidate.funding_id
                ...
                && stored.absolute_timelock == candidate.absolute_timelock
                && stored.evidence_hash == candidate.evidence_hash
                && candidate.confirmation_count >= stored.confirmation_count
        }
        _ => false,
    }
}
...
mod tests {
    ...
    #[test]
    fn terminal_dropped_sends_atomically_release_activated_reservations() {
        ...
        let observed = store
            .load_workflow::<HnsSendWorkflow>(workflow_id)
            .expect("load observed workflow")
            .expect("observed workflow");
        persist_observed_send_stage(
            &mut store,
            &config,
            &observed,
            HnsSendStage::Conflicted,
        ...
        .expect("persist conflict and release reservation");
        let observed = store
            .load_workflow::<HnsSendWorkflow>(workflow_id)
            .expect("reload observed workflow")
            .expect("observed workflow remains auditable");
        assert_eq!(observed.state.stage, HnsSendStage::Conflicted);
        assert!(
        ...
        let obsolete_workflow_id = WorkflowId::new([48; 16]);
        let mut obsolete_state = state;
        obsolete_state.plan.workflow_id = obsolete_workflow_id;
        let mut obsolete_reservation = reservation;
        obsolete_reservation.workflow_id = obsolete_workflow_id;
        store
            .save_workflow(
                obsolete_workflow_id,
                WorkflowKind::HnsSend,
                0,
                &obsolete_state,
                true,
        ...
        store
            ...
            .expect("obsolete activated reservation");
        ...
        let stored = store
            .load_workflow::<HnsSendWorkflow>(obsolete_workflow_id)
    ...
    #[test]
    fn settlement_retry_terms_accept_only_monotonic_confirmation_refreshes() {
        let stored_lock = VerifiedLock {
            module: ModuleId::Handshake,
            session_id: SessionId::new([101; 32]),
            funding_id: TransactionHash::new([102; 32]),
            amount: Amount::new(WalletAsset::Hns, 546),
            hashlock: ObjectHash::new([103; 32]),
            absolute_timelock: 700,
            confirmation_count: 2,
            evidence_hash: ObjectHash::new([104; 32]),
        };
        ...
        assert!(settlement_terms_are_retry_compatible(
            ...
            &HnsSettlementTerms::Redeem {
                lock: refreshed_lock.clone(),
            },
        ));

        let mut regressed_lock = stored_lock.clone();
        regressed_lock.confirmation_count = 1;
        assert!(!settlement_terms_are_retry_compatible(
            &HnsSettlementTerms::Redeem {
                lock: stored_lock.clone(),
            },
