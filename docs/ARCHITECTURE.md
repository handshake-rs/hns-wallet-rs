...
| `hns-wallet-mobile` | platform-neutral lifecycle, synchronized-read, and trusted-native value controllers over one exact shared store; atomic single-HNS-account bootstrap, private ABI-v2 lifecycle controls, minimized read/name projections, closed process-local HNS value approvals, wallet-peer Shakedex board operations, and a direct-Denuo façade whose listener admission/connect time comes from the value runtime | raw platform keys, a concrete device backend, generated JNI/C/Kotlin/Swift bindings, WebView/provider entry points, generic provider JSON, or product transport orchestration |
...
| `hns-wallet-service` | random service/wallet sessions, exact sequences, private host control, permission-backed provider composition, locked existing-database control runtime, same-Arc exact-account read/value library runtimes, encrypted-profile bootstrap with private internal unlock, exact-six read admission plus one native-only HNS authority-evidence operation, closed trusted-native value approvals and wallet-peer Shakedex operations, profile/account revision revalidation and failure/drop locking, runtime-scoped capability advertisement, atomic account grant, singleton persisted-account recheck, scoped public read projections | cross-process database or HRM/HNSA namespace brokerage, one-shot secret transport, browser engine policy, generic native/provider value entry points, or product availability claims |
...
The board-specific HNS join captures the selected wallet's complete encrypted
`WalletAccount` prefix before node or clock work. It consumes and refreshes that
ciphertext-fingerprinted lease in the same coherent snapshot as the related
board read. Read-only results may retain that refreshed context, but the public
`verify_unchanged_account` check returns no write guard and is not atomic with a
later mutation. Offer and cancellation writes instead consume the refreshed
lease as a second cross-kind compare-only guard beside the primary board
namespace lease in the same immediate transaction.
...
sequence/phase transition before sync, commits each returned update to a
versioned encrypted BDK changeset entity, reconciles encrypted
transaction/output mirrors in bounded chunks, and commits ready last. Crash
recovery can resume an exact pending reconciliation from the durable BDK tip;
a tip ahead of a still-synchronizing journal forces recovery. A bounded set of
previous hashes is checked against Kyoto's local most-work chain to identify
reorg ancestry; an unbounded/deep mismatch requires recovery.
