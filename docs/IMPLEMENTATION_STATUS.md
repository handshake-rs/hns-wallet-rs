...
| Mobile controller | platform-neutral lifecycle API with owned zeroizing 32-byte database key and recovery input, guarded exact-24-word create/restore, complete seed/account bootstrap open, one shared store authority, locked startup, private ABI-v2 negotiation, status/unlock/lock/single-account controls, and fail-closed request/session handling; separate backend/clock-injected read controller composes `HnsAccountReadRuntime` and `PersistentHnsReadRuntime` around that literal authority and returns a bounded serialized balance/canonical-receive/name-alias/history/minimized-known-name/successful-tip-status snapshot; the mobile boundary requires the name alias to equal the canonical account-zero target; direct trusted-native `import_name_exact_text()` returns the minimized name summary; a script-free chain snapshot and bound genesis check precede all ScriptId derivation/query; browser/provider capabilities are rejected; native direct-HNS, Shakedex, Bitcoin, and atomic-swap composition is available behind private trusted-mobile permits | installed-device, live-network, resource, store, browser-provider, and cross-chain settlement qualification remain product-owned |
| Testkit | deterministic non-mainnet, hostile-input, reorg, and qualification fixtures | full multi-process network harnesses |
| Browser products | authority and adapter work lives in separate repositories; this wallet producer does not project the native compatibility name alias into website/provider JSON | `hns-dane-browser-mobile` consumes HNWR-v2 and validates `nameReceiveTarget` as the canonical target through its trusted native surface; the Chromium provider join remains unavailable, and installed-extension/native-host plus signed-device qualification stay product-owned |
