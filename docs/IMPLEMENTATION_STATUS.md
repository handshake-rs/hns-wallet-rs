...
Snapshot: 2026-08-10. The production-hardening source boundary is implemented;
...
- canonical `hns-swap` 0.2 source is pinned to immutable release-candidate
  revision `abf11ff`; wallet publication requires all 17 corresponding
...
That evidence belongs to the exact implementation commit. The later release
metadata and protocol-repin tranche requires its own CI and manual normalized
package preflight. Neither predecessor success nor package preparation covers
multi-process regtest/restart/reorg, installed products, real networks,
benchmarks, resource measurement, or independent audit, and it does not enable
any value or settlement constant. The canonical current ledger is
