...
| --- | --- | --- |
| Standalone workspace | 14 crates, resolver 3, Rust 1.89, independent lockfile, no sibling paths; historical locked CI, CodeQL, and normalized preflight evidence remains recorded; current source uses published registry `hns-rs` `0.3.1` source `0e99addca59778b7b7c6fc56291333a97c4c8815` and published registry `hns-dane-engine` `0.2.2` source `b7fdf8826c81b77650a0f740d1f05314b74969f9`, with exact archive manifests in `release/` | exact wallet CI, CodeQL, and manually dispatched normalized release preflight; authorized wallet publication, tag, and registry verification |
...
| Denuo market | direct-offer protocol and durable wallet core; product unavailable | Direct signed HNS/BTC offers, cancellations, exact live-offer grouping, and durable direct-session admission replace the removed reporter/source price-round system. The board is only peer discovery; the signed offer remains the sole authority for the HNS and BTC amounts. |
| Bitcoin | BDK BIP84 create/load/receive/send primitives; strict aggregate BDK-3.1.0 changeset persisted under encrypted account-authenticated CAS; one shared store/key authority for BDK state, scan journal, mirrors, broadcast, and bounded per-session HTLC watches; swap-evidence-before-BDK-checkpoint crash ordering; context-bound atomic-swap allocation keys with authenticated re-derivation; bounded Kyoto tip discovery and supervisor; one direct-P2P BIP157 stream for descriptor and active HTLC scripts; canonical funding/spend discovery, reorg-aware confirmations, monotonic preimage retention; exact signed redeem/refund verification and fee-bound durable pre-broadcast journal | normalized or authenticated chunked BDK backend beyond the current 1 MiB aggregate limit; explicit legacy BDK SQLite import tool; cross-chain coordinator and installed-product integration; watch/record archival; regtest/restart/reorg/adversarial product qualification and benchmarks; value gate remains false until the complete product flow is connected |
...
The current dependency graph uses published registry `hns-rs` `0.3.1` source
`0e99addca59778b7b7c6fc56291333a97c4c8815`, with all 19 upstream archive
checksums in `release/hns-rs-0.3.1-crates.sha256`, and published registry
...
4,096-record lifetime caps, complete coordinator/restart qualification,
regtest/reorg/adversarial evidence, trusted-time policy, and resource
measurements remain. The domain-separated keys and encrypted monotonic allocation source
