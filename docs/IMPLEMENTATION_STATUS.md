...
| Denuo market | pinned canonical name-market and price-round envelopes; bounded replay/tombstone-safe encrypted fixed-price board with sequence watermarks and CAS restart validation; bounded encrypted/CAS offline V2 offer/cancellation outbox with exact-envelope and request/content identity, schema-v3 persist-before-return single-flight handoff, explicit outcome-unknown crash recovery, bounded retry/exhaustion, immutable schema-v1 acknowledgement compatibility, and terminal endpoint-signed `RelayAccepted` receipts that bind the exact handoff plus wallet-supplied HRM/HNSA policy and self-validate after restart; schema v2 cannot inject acknowledgement or acceptance, exact receipt replay is revision-stable, and conflicting terminal receipts fail closed; exact-policy-bound encrypted zero-ID price-round gossip cache with optional predecessor-checkpoint bootstrap, trusted local `accepted_at_unix`, durable canonical reporter-aligned sequence high-watermarks, full retained-row/link validation, and atomic retirement from a maximum-128-round suffix; retired round hashes/IDs leave duplicate detection while reporter high-watermarks remain; chain-neutral reservations/sessions; no receipt-provided current HRM/HNSA, board, quote, value, or live-chain authority and no gate changes | current HRM/HNSA authority adapter, authenticated live transport and relay supervision, board admission/reacquisition from runtime-owned current unspent lock evidence, peer policy and reporter governance, current chain-anchor authority, quote/value integration, product integration and qualification; every Denuo and value gate remains `false` |
