...
impl Peer {
    ...
    async fn publish_pending_transactions<W>(
        &mut self,
        writer: &mut W,
        ...
        W: AsyncWrite + Send + Unpin,
    {
        let (wtxids, packages) = {
            ...
            (queue.pending_wtxid(), queue.pending_packages())
        };
        if wtxids.is_empty() {
            return Ok(());
        }
        let inventory = message_generator.announce_transactions(wtxids);
        self.write_bytes(writer, inventory).await?;
        for (advertised_wtxid, transactions) in packages {
            for transaction in transactions {
                let wtxid = transaction.compute_wtxid();
                let payload = message_generator.broadcast_transaction(transaction);
                self.write_bytes(writer, payload).await?;
                self.message_state.sent_tx(wtxid);
            }
            self.tx_queue
                .lock()
                .await
                .sent_package_payload(advertised_wtxid, self.nonce);
        }
        Ok(())
    }
                    ...
                    match inv {
                        Inventory::WTx(wtxid) => {
                            ...
                            if let Some(transaction) = transaction {
                                ...
                                self.message_state.sent_tx(wtxid);
                                tx_queue.sent_transaction_payload(wtxid, self.nonce);
                            }
                        }
        ...
        W: AsyncWrite + Send + Unpin,
    {
        ...
        match request {
            ...
            MainThreadMessage::BroadcastPending => {
                ...
                self.publish_pending_transactions(writer, message_generator)
                    .await?;
            }
