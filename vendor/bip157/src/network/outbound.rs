...
pub(in crate::network) fn make_version(port: Option<u16>, network: &Network) -> VersionMessage {
    ...
    VersionMessage {
        ...
        start_height: 0,
        // This light client does not need the remote mempool, but value
        // operations do require a normal transaction-relay connection. A
        // false relay flag leaves Bitcoin Core without per-peer TxRelay state
        // and makes the connection a poor publication path.
        relay: true,
    }
}
